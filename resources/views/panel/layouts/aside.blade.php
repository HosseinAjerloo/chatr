            ...
            @foreach($activeUsers as $active)
                <li
                    ...
                    <div class="w-[50px] h-[50px] border border-2 border-black rounded-full relative">
                        <img src="{{asset(\App\Models\User::find($active->user_id)->photo?->path?\App\Models\User::find($active->user_id)->photo?->path:'chartFront/assets/images/avatar.jpg')}}" alt="" class="rounded-full w-full h-full" />
                        <div
